...
public class DateTimeRangeWidget extends Composite {
  ...
  public static DateTimeRangeWidget makeForEventContentItem(EventContentItem event) {
    Date eventDateTime = event.getEventDate();
    
    if (eventDateTime == null) {
      // If there is no special date on the event, just show the creation time
      // as the event time.
      eventDateTime = event.getTimestamp();
    }
    
    DateTimeRangeWidget widget = new DateTimeRangeWidget();
    widget.setDateTime(eventDateTime, null);
    return widget;
  }
